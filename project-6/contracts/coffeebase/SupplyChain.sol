...
contract SupplyChain is Ownable, FarmerRole, DistributorRole, RetailerRole, ConsumerRole {
  ...
  modifier verifyCaller (address _address) {
    require(msg.sender == _address, "You can't call this.");
    _;
  }

  // Define a modifier that checks if the paid amount is sufficient to cover the price
  modifier paidEnough(uint _price) {
    require(msg.value >= _price, "Didn't payed enough");
    _;
  }
  ...
  modifier harvested(uint _upc) {
    require(items[_upc].itemState == State.Harvested, "The item is not Harvested");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Processed
  modifier processed(uint _upc) {
    require(items[_upc].itemState == State.Processed, "The item is not Processed");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Packed
  modifier packed(uint _upc) {
    require(items[_upc].itemState == State.Packed, "The item is not Packed");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is ForSale
  modifier forSale(uint _upc) {
    require(items[_upc].itemState == State.ForSale, "The item is not ForSale");
    _;
  }
  ...
  modifier shipped(uint _upc) {
    require(items[_upc].itemState == State.Shipped, "The item is not Shipped");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Received
  modifier received(uint _upc) {
    require(items[_upc].itemState == State.Received, "The item is not Received");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Purchased
  modifier purchased(uint _upc) {
    require(items[_upc].itemState == State.Purchased, "The item is not Purchased");
    _;
  }
  ...
  itemSKU = returnItem.sku;
  itemUPC = returnItem.upc;
  ownerID = returnItem.ownerID;
  originFarmerID = returnItem.originFarmerID;
  originFarmName = returnItem.originFarmName;
  originFarmInformation = returnItem.originFarmInformation;
  originFarmLatitude = returnItem.originFarmLatitude;
  originFarmLongitude = returnItem.originFarmLongitude;
  ...
  productNotes = returnItem.productNotes;
  productPrice = returnItem.productPrice;
  itemState = uint(returnItem.itemState);
  distributorID = returnItem.distributorID;
  retailerID = returnItem.retailerID;
  consumerID = returnItem.consumerID;
